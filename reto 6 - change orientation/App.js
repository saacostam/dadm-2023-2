...
import {StyleSheet, Text, View, BackHandler, Modal, Pressable, TouchableHighlight} from 'react-native';
...
export default function App() {
    ...
    useCallback(() => {
        AsyncStorage.default.getItem('one').then(
            (value) => {
                setScore(prevState => {
                        return {
                            ...prevState,
        ...
        AsyncStorage.default.getItem('two').then(
            (value) => {
                setScore(prevState => {
                        return {
                            ...prevState,
        ...
        AsyncStorage.default.getItem('tie').then(
            (value) => {
                setScore(prevState => {
                        return {
                            ...prevState,
    ...
    const analyzeGrid = async (grid) => {
        ...
        const checkAndUpdateRow = ([ONE, TWO, THREE]) => {
            if (ONE.content === TWO.content && TWO.content === THREE.content && ONE.content !== FIELD_FILLER.EMPTY) {
                ...
                if (ONE.content === FIELD_FILLER.ONE){
                    AsyncStorage.default.setItem('one', `${score.ONE + 1}`).then();
                }else{
                    AsyncStorage.default.setItem('two', `${score.TWO + 1}`).then();
                }
            }
        }
        ...
        const checkGameOver = () => {
            ...
            if (allFilled && !gameOver) {
                ...
                AsyncStorage.default.setItem('tie', `${score.TIE+1}`).then();
            }
        }
    ...
    <View style={styles.container}>
          <View
            style={{
                ...
                display: 'flex',
                flexDirection: (orientation === Orientation.PORTRAIT_UP || orientation === Orientation.PORTRAIT_DOWN) ? 'column' : 'row',
            }}
          >
              <View
                style={{
                    marginBottom: (orientation === Orientation.PORTRAIT_UP || orientation === Orientation.PORTRAIT_DOWN) ? 32 : 0,
                }}
              >
                  {
              ...
              <View
                style={{
                    display: 'flex',
                    flexDirection: (orientation === Orientation.PORTRAIT_UP || orientation === Orientation.PORTRAIT_DOWN) ? 'row' : 'column',
                    alignItems: 'space-between',
                    justifyContent: 'center',
                    marginHorizontal: (orientation === Orientation.LANDSCAPE_LEFT || orientation === Orientation.LANDSCAPE_RIGHT) ? 48 : 0,
                    marginVertical: (orientation === Orientation.LANDSCAPE_LEFT || orientation === Orientation.LANDSCAPE_RIGHT) ? 0 : 16,
                }}
              >
                  <Text style={styles.counter} >
                      {`${FIELD_FILLER.ONE}: ${score.ONE}`}
                  </Text>
                  <Text style={styles.counter}>
                      {`${FIELD_FILLER.TWO}: ${score.TWO}`}
                  </Text>
                  <Text style={styles.counter} >
                      {`TIE: ${score.TIE}`}
                  </Text>
                  <TouchableHighlight
                    ...
                    onPress={() => {
                        setScore({ONE: 0, TWO: 0, TIE: 0});
                        AsyncStorage.default.setItem('one', '0').then();
                        AsyncStorage.default.setItem('two', '0').then();
                        AsyncStorage.default.setItem('tie', '0').then();
                    }}
                  >
                      <Text style={{
                          fontSize: 18,
